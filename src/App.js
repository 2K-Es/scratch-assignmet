...
import { setList } from "./redux/midarea/list";

function App(props) {
  const complist = useSelector((state) => state.list);
  const dispatch = useDispatch();
  const setListAction = (id, list) => dispatch(setList({ id, list }));
  ...
  const onDragEnd = (result) => {
    ...
    let old_list = structuredClone(complist.midAreaLists);
    ...
    if (dest_index > -1)
      setListAction(result.destination.droppableId, old_list[dest_index].comps);
    if (source_index > -1)
      setListAction(result.source.droppableId, old_list[source_index].comps);
  };
